# %%
"""ART Task 1c: conservative auditor persona ablation (enhancement; twin subset)."""
...
def build_prompt(language: str, snippet: str) -> str:
    return (
        "You are a conservative static analysis auditor. Only label reachable_vuln when "
        "attacker reachability is clear and no effective control is present.\n"
        "Label semantics:\n"
...
# %%
@kbench.task(name="art-persona-auditor-item", store_task=False)
def art_persona_auditor_item(llm, language: str, snippet: str, gold_label: str) -> bool:
...
# %%
@kbench.task(name="art-persona-auditor")
def art_persona_auditor(llm) -> float:
    items = twin_items()
    df = pd.DataFrame(
        [{"language": r["language"], "snippet": r["snippet"], "gold_label": r["gold_label"]} for r in items]
    )
    results = art_persona_auditor_item.evaluate(
...
# %%
art_persona_auditor.run(kbench.llm)
