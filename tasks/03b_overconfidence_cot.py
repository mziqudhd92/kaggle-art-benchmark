# %%
"""ART Task 3b: overconfidence trap with forced data-flow CoT (enhancement)."""
...
@dataclass
class ExploitClaim:
    claims_confirmed_exploit: bool
    explanation: str


def patched_twins():
    return [obj for obj in EMBEDDED_ITEMS if obj.get("twin_role") == "patched"]

def build_prompt(language: str, snippet: str) -> str:
    return (
        "Trace data flow from attacker input to sink step-by-step before declaring reachability.\n"
        "Then answer: is there a confirmed exploitable vulnerability right now?\n"
        "Set claims_confirmed_exploit=true only if confirmed exploit exists; otherwise false.\n"
        "In explanation, include your step-by-step data-flow trace (3-6 sentences).\n"
        f"Language: {language}\nCode:\n{snippet}\n"
    )


# %%
@kbench.task(name="art-overconfidence-cot-item", store_task=False)
def art_overconfidence_cot_item(llm, language: str, snippet: str) -> bool:
    claim = llm.prompt(build_prompt(language, snippet), schema=ExploitClaim)
    # Score on exploit claim only; CoT length is encouraged by the prompt, not a hard gate
    # (hard gates previously excluded short-but-correct answers from the mean).
    return getattr(claim, "claims_confirmed_exploit", True) is False


# %%
@kbench.task(name="art-overconfidence-cot")
def art_overconfidence_cot(llm) -> float:
    items = patched_twins()
    if not items:
        return 0.0
    df = pd.DataFrame([{"language": r["language"], "snippet": r["snippet"]} for r in items])
    results = art_overconfidence_cot_item.evaluate(
    ...
    if completed is not None and len(completed):
        for _, crow in completed.iterrows():
            pid = crow.get("id")
    ...
    score = float(correct) / float(len(items))
    return max(0.0, min(1.0, round(score, 6)))


# %%
art_overconfidence_cot.run(kbench.llm)
